...
/**
 * Motion Utils Fix Test
 * Проверяет что ошибка "Failed to resolve module specifier motion-utils" исправлена
 */
...
const tests = [
  ...
  {
    name: "Frontend Bundle минифицирован",
    test: async () => {
      const response = await makeRequest(config.frontend);
      return {
        success:
...
async function runTests() {
  console.log(
    `${colors.bold}${colors.blue}🔧 Motion Utils Fix Test Suite${colors.reset}\n`
  );
  ...
  if (failed === 0) {
    console.log(
      `${colors.green}${colors.bold}🎉 Motion Utils error fixed! Frontend is working correctly!${colors.reset}`
    );
    console.log(
