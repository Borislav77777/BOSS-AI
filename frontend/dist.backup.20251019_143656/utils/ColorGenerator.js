...
export class ColorGenerator {
     ...
     */
    static generateSurfaceColor(background) {
     ...
     */
    static generateBorderColor(background, isLight) {
        // const _hsl = this.hexToHsl(background);
     ...
     */
    static generateButtonColors(primary, background) {
        const primaryHsl = this.hexToHsl(primary);
        const backgroundHsl = this.hexToHsl(background);
        const isLight = backgroundHsl.l > 50;
        return {
            primary: primary,
            secondary: this.hslToHex(primaryHsl.h, primaryHsl.s, isLight ? Math.min(primaryHsl.l + 20, 90) : Math.max(primaryHsl.l - 20, 10)),
            hover: this.hslToHex(primaryHsl.h, primaryHsl.s, isLight ? Math.max(primaryHsl.l - 10, 20) : Math.min(primaryHsl.l + 10, 80))
        };
    }
    /**
    ...
    static generateColorScheme(primaryColor) {
        ...
        const background = this.hslToHex(_hsl.h, Math.min(_hsl.s + 10, 100), Math.max(_hsl.l - 40, 5));
     ...
     */
    static createCustomThemeVariables(primaryColor) {
     ...
     */
    static getContrastIconColor(backgroundColor) {
     ...
     */
    static getContrastIconColorForAccent(accentColor, isDarkTheme = true) {
     ...
     */
    static getCustomVariablesToRemove() {
        return [
