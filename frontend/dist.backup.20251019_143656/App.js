...
import SalesScriptsManager from '@/components/SalesScripts/SalesScriptsManager';
import { AILawyerService } from '@/components/Services/AILawyer';
import { OzonManager } from '@/components/Services/OzonManager';
import { PhotoStudioService } from '@/components/Services/PhotoStudio/PhotoStudioService';
import { PromptsService } from '@/components/Services/Prompts';
import { SettingsModular } from '@/components/Settings/SettingsModular';
import { Sidebar } from '@/components/Sidebar';
import { WidgetsServiceTabs } from '@/components/Widgets/WidgetsServiceTabs';
...
const AppContent = memo(() => {
    ...
    const { state, updateSettings, switchSection, dispatch } = platformContext;
    ...
    // Открытие панели Промптов по событию
    useEffect(() => {
        const handler = () => {
            if (state.layout.activeSection === 'prompts') {
    ...
    }, [state.layout.activeSection, switchSection]);
    ...
    if (state.isInitialized && !state.authUser) {
        return (_jsx(AnimatePresence, { mode: "wait", children: _jsx(AuthWidget, { onAuthSuccess: async (user, token) => {
                    // Обработка успешной авторизации
                    console.log('[App] Авторизация успешна:', user);
                    // Не делаем повторный вызов loginWithTelegram (он уже был выполнен внутри AuthWidget)
                    // Диспатчим пользователя в контекст, чтобы скрыть AuthWidget и показать приложение
    ...
    return (_jsxs(motion.div, { className: "min-h-screen w-screen flex app-container", initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -20 }, transition: { duration: 0.6, ease: "easeOut" }, children: [_jsx("div", { className: "fixed top-0 left-0 right-0 h-1 z-50 animated-top-bar" }), _jsx(motion.div, { className: cn("sidebar-container relative sidebar-dynamic-width", state.layout.sidebarCollapsed && "collapsed"), initial: { opacity: 0, x: -20, scale: 0.98 }, animate: { opacity: 1, x: 0, scale: 1 }, transition: {
                ...
                }, children: _jsx(Sidebar, {}) }), _jsxs(motion.div, { className: "flex-1 flex h-screen main-content", initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.5, delay: 0.2 }, children: [_jsx(motion.div, { className: cn("flex-1 overflow-x-hidden main-content-scrollbar rounded-2xl expanded-tabs-container", state.layout.activeSection === 'widgets' && "service-tabs-expanded"), initial: { opacity: 0, scale: 0.95 }, animate: { opacity: 1, scale: 1 }, transition: { duration: 0.5, delay: 0.4 }, style: { '--expanded-height': `${state.layout.expandedTabsHeight}px` }, children: state.layout.activeSection === 'ozon-manager' ? (_jsx(OzonManager, {})) : state.layout.activeSection === 'ai-lawyer' ? (_jsx(AILawyerService, {})) : state.layout.activeSection === 'corporate' ? (_jsx(CorporateWebsite, {})) : state.layout.activeSection === 'sales-scripts' ? (_jsx(SalesScriptsManager, {})) : state.layout.activeSection === 'regulations' ? (_jsx(RegulationsManager, {})) : state.layout.activeSection === 'photo-studio' ? (_jsx(PhotoStudioService, {})) : (_jsx(Workspace, {})) }), state.layout.activeSection === 'settings' && (_jsx(motion.div, { className: "settings-panel relative rounded-2xl full-height", initial: { opacity: 0, x: 10, scale: 0.99 }, animate: { opacity: 1, x: 0, scale: 1 }, transition: { duration: 0.3, delay: 0.2 }, style: { width: 500 }, children: _jsx(SettingsModular, {}) })), state.layout.activeSection === 'prompts' && (_jsx(motion.div, { className: "prompts-panel relative rounded-2xl full-height", initial: { opacity: 0, x: 10, scale: 0.99 }, animate: { opacity: 1, x: 0, scale: 1 }, transition: { duration: 0.3, delay: 0.2 }, style: { width: 800 }, children: _jsx(PromptsService, {}) })), state.layout.chatVisible && (_jsxs(motion.div, { className: "chat-container relative rounded-2xl h-screen max-h-screen", style: { '--chat-width': `${state.layout.chatWidth}px` }, initial: { opacity: 0, x: 50 }, animate: { opacity: 1, x: 0 }, transition: { duration: 0.5, delay: 0.4 }, children: [_jsx(Chat, { onInputResize: handleChatInputResizeMouseDown }), _jsx("div", { className: "chat-resizer", onMouseDown: handleChatResizeMouseDown, title: "\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u0448\u0438\u0440\u0438\u043D\u0443 \u0447\u0430\u0442\u0430" })] }))] }), _jsx(WidgetsServiceTabs, { onTranscriptionComplete: (text) => {
