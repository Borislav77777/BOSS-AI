...
class PromptManager {
    ...
    async searchPrompts(query, filters) {
        ...
        for (const module of this.modules.values()) {
            ...
            if (filters?.source && module.source !== filters.source)
                continue;
            if (filters?.moduleId && module.id !== filters.moduleId)
                continue;
            for (const prompt of module.prompts) {
                // Применяем фильтры к промпту
                if (filters?.categoryId && prompt.categoryId !== filters.categoryId)
                    continue;
                if (filters?.folderId && prompt.folderId !== filters.folderId)
                    continue;
                if (filters?.system !== undefined && prompt.system !== filters.system)
                    continue;
                if (filters?.tags && !filters.tags.some(tag => prompt.tags.includes(tag)))
                    continue;
                // Поиск по тексту
    ...
    async getPromptsInCategory(categoryId) {
        const prompts = [];
        for (const module of this.modules.values()) {
            for (const prompt of module.prompts) {
    ...
    async getPromptsInFolder(folderId) {
        const prompts = [];
        for (const module of this.modules.values()) {
            for (const prompt of module.prompts) {
    ...
    // Быстрый доступ
    async addToQuickAccess(promptId) {
    ...
    async removeFromQuickAccess(promptId) {
    ...
    async createUserModule(name, description) {
    ...
    async ensureBossAIGenerationCategory() {
...
4. Рекомендации`;
    ...
    async validateImprovedPrompt(improvedPrompt, userFeedback) {
        try {
...
Учти эти требования при выполнении задачи.`;
    ...
    async createUserCategory(name, description, icon, color) {
        try {
    ...
    async createPromptInCategory(categoryId, title, body, description, tags, currentProject) {
        try {
    ...
    async movePromptToCategory(promptId, newCategoryId) {
        try {
            const prompt = await this.getPrompt(promptId);
            if (!prompt) {
    ...
    async mergeGeneratedCategories() {
        try {
            const allCategories = await this.getCategories();
            const generatedCategories = allCategories.filter(c => c.title === 'Сгенерированные' || c.title === 'Boss AI Generation');
