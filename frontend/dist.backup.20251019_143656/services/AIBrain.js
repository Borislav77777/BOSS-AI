...
class AIBrainService {
     ...
     */
    async analyzeIntentWithContext(message, chatContext) {
    ...
    async analyzeIntent(message) {
        ...
        for (const service of serviceCommands) {
            const matchCount = service.keywords.filter(keyword => lowerMessage.includes(keyword)).length;
            if (matchCount > 0) {
     ...
     */
    async handleSystemCommand(message, _intent) {
        const lowerMessage = message.toLowerCase();
     ...
     */
    async handleGeneralQueryWithContext(message, intent, chatContext) {
     ...
     */
    buildContextInfo(chatContext) {
        const parts = [];
        if (chatContext.activeProject) {
            parts.push(`Активный проект: ${chatContext.activeProject.title}`);
        }
        if (chatContext.attachedItems.length > 0) {
            const items = chatContext.attachedItems.map((item) => `${item.type}: ${item.title}${item.content ? `\nСодержимое: ${item.content.substring(0, 200)}...` : ''}`).join('\n\n');
     ...
     */
    async handleGeneralQuery(_message, _intent) {
     ...
     */
    updateContext(activeService, activeSection) {
