...
export const ProgressIndicator: React.FC<ProgressIndicatorProps> = memo(({
    ...
    const getIcon = () => {
        switch (type) {
            case 'loading':
                return <Loader2 className="w-5 h-5 animate-spin text-text" />;
            case 'success':
    ...
    return (
        <div className={cn(
            "fixed top-4 right-4 z-50 flex items-center space-x-3 px-4 py-3 rounded-lg border shadow-lg backdrop-blur-sm",
            getBackgroundColor(),
            className
        )}>
            {getIcon()}
