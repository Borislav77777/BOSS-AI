...
export interface UnifiedButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    ...
    loading?: boolean;
    isLoading?: boolean; // Алиас для совместимости с AnimatedButton
    icon?: React.ReactNode;
    leftIcon?: React.ReactNode; // Алиас для совместимости с AnimatedButton
    rightIcon?: React.ReactNode; // Алиас для совместимости с AnimatedButton
...
export const UnifiedButton: React.FC<UnifiedButtonProps> = memo(({
    variant = 'primary',
    size = 'md',
    rounded = 'md',
    loading = false,
    ...
    const animatedStyle = isAnimatedVariant ? animatedButtonVariants[variant as keyof typeof animatedButtonVariants] : null;

    const baseClasses = cn(
        'platform-button-base',
        !isAnimatedVariant && basicVariantClasses[variant as keyof typeof basicVariantClasses],
        sizeClasses[size],
        roundedClasses[rounded],
        isActuallyLoading && 'platform-button-loading',
        disabled && 'platform-button-disabled',
        className
    );
    ...
    const motionProps: MotionProps = isAnimatedVariant ? {
        initial: 'initial',
        whileHover: 'hover',
        whileTap: 'tap',
    ...
    return (
        ...
        >
            ...
            {isActuallyLoading && (
                <motion.div
                    className="platform-button-spinner"
                    animate={{ rotate: 360 }}
            ...
            {/* Left icon */}
            {!isActuallyLoading && actualIcon && iconPosition === 'left' && (
                <motion.div
                    initial={{ opacity: 0, x: -10 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.1 }}
                    className="platform-button-icon-left"
                >
                    {actualIcon}
                </motion.div>
            )}
            ...
            {/* Right icon */}
            {!isActuallyLoading && actualRightIcon && iconPosition === 'right' && (
                <motion.div
                    initial={{ opacity: 0, x: 10 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.1 }}
                    className="platform-button-icon-right"
                >
                    {actualRightIcon}
                </motion.div>
            )}

            {/* Ripple effect для анимированных вариантов */}
            {isAnimatedVariant && (
                <motion.div
