...
export class ConnectorValidator {
  private static instance: ConnectorValidator;
  private testResults: Map<string, ConnectorTestResult[]> = new Map();

  private constructor() {
    // Инициализация валидатора
  }

  public static getInstance(): ConnectorValidator {
   ...
   */
  public validateConnectorConfig(config: ConnectorConfig): ValidationResult {
    const errors: string[] = [];
   ...
   */
  public validateServiceConfig(config: ServiceConfig): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    const suggestions: string[] = [];
   ...
   */
  public async testConnector(serviceId: string, config: ConnectorConfig): Promise<ConnectorTestResult[]> {
    const results: ConnectorTestResult[] = [];
   ...
   */
  public getTestResults(serviceId: string): ConnectorTestResult[] {
   ...
   */
  public getAllTestResults(): Map<string, ConnectorTestResult[]> {
   ...
   */
  public clearTestResults(serviceId?: string): void {
    if (serviceId) {
  ...
  private addAuthHeaders(headers: Record<string, string>, auth: ConnectorConfig['auth']): void {
    if (!auth) return;
