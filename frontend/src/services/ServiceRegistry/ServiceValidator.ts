...
export class ServiceValidator {
  ...
  private readonly REQUIRED_FIELDS = ['id', 'name', 'version', 'category'];
  ...
  private validateRequiredFields(service: ServiceConfig, errors: string[]): void {
    this.REQUIRED_FIELDS.forEach(field => {
      if (!(field in service) || service[field as keyof ServiceConfig] === undefined) {
        errors.push(`Отсутствует обязательное поле: ${field}`);
      }
    });
  }

  private validateDataTypes(service: ServiceConfig, errors: string[]): void {
  ...
  private validateValues(service: ServiceConfig, errors: string[], warnings: string[]): void {
    // Проверка ID
  ...
  private validateTools(service: ServiceConfig, errors: string[], warnings: string[]): void {
    if (!Array.isArray(service.tools)) return;
  ...
  private validateSettings(service: ServiceConfig, errors: string[], _warnings: string[]): void {
    if (service.settings && typeof service.settings !== 'object') {
      errors.push('Настройки должны быть объектом');
    }
  }

  private validateChatButtons(service: ServiceConfig, errors: string[], _warnings: string[]): void {
    if (!service.chatButtons) return;
  ...
  private validateTheme(service: ServiceConfig, errors: string[], warnings: string[]): void {
    if (service.theme) {
      if (!service.theme.id) {
  ...
  private generateSuggestions(service: ServiceConfig, suggestions: string[]): void {
    // Предложения по улучшению
