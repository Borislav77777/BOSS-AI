...
export default {
   ...
   */
  async chatCompletion(params: Record<string, unknown> = {}) {
    const { prompt, userInput } = params;
    const message: string = String(prompt ?? userInput ?? 'Привет!');
   ...
   */
  async rewriteText(params: Record<string, unknown> = {}) {
    const { prompt, userInput } = params;
    const text: string = String(prompt ?? userInput ?? '');
   ...
   * Краткое изложение
   */
  async summarizeText(params: Record<string, unknown> = {}) {
    const { prompt, userInput } = params;
    const text: string = String(prompt ?? userInput ?? '');
   ...
   * Перевод текста
   */
  async translateText(params: Record<string, unknown> = {}) {
    const { prompt, userInput } = params;
    const text: string = String(prompt ?? userInput ?? '');
