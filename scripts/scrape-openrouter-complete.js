...
/**
 * ПОЛНЫЙ парсер OpenRouter - ВСЕ модели со скриншотов
 * Собирает АБСОЛЮТНО ВСЕ названия из всех 13 скриншотов
 */

// ВСЕ модели из ВСЕХ скриншотов (включая без "(free)" в названии)
const allModelsFromScreenshots = [
  // Скриншот 1: Nous, DeepSeek модели
  ...
  // Скриншот 2: Agentica, Tongyi, Venice, TNG, NVIDIA, MiniMax, Meituan, Google
  ...
  { name: "Google: Gemini 2.5 Flash Image Preview (Nano Banana)", provider: "Google", modelName: "Gemini 2.5 Flash Image Preview (Nano Banana)" },

  // Скриншот 3: Qwen, Microsoft, Shisa AI, ArliAI, Meta
  ...
  // Скриншот 4: Qwen, Mistral, Google, Meta
  ...
  // Скриншот 5: Qwen, Mistral, Google, Meta
  ...
  // Скриншот 6: Qwen, Mistral, Google, Meta
  ...
  { name: "AlfredPros: CodeLLaMa 7B Instruct Solidity (free)", provider: "AlfredPros", modelName: "CodeLLaMa 7B Instruct Solidity" },
  ...
  // Скриншот 7: Deep Cogito, Nous, Arcee AI, DeepSeek, Relace
  ...
  { name: "Nous: DeepHermes 3 Llama 3 8B Preview (free)", provider: "Nous", modelName: "DeepHermes 3 Llama 3 8B Preview" },
  { name: "NousResearch: Hermes 2 Pro - Llama-3 8B (free)", provider: "NousResearch", modelName: "Hermes 2 Pro - Llama-3 8B" },
  ...
  // Скриншот 8: DeepSeek, Arcee AI, Cohere
  ...
  { name: "Cogito V2 Preview Llama 109B (free)", provider: "Deep Cogito", modelName: "Cogito V2 Preview Llama 109B" },
  ...
  // Скриншот 9: Arcee AI, DeepSeek, Cohere
  ...
  // Скриншот 10: Cohere, SorcererLM, Inflection, Nous
  ...
  // Скриншот 11: OpenAI, Agentica, Perplexity, Google, Tongyi, AI21
  ...
  // Скриншот 12: TNG, Morph, Google, Microsoft, OpenAI
  { name: "TNG: DeepSeek R1T2 Chimera (free)", provider: "TNG", modelName: "DeepSeek R1T2 Chimera" },
  { name: "Morph: Morph V3 Large (free)", provider: "Morph", modelName: "Morph V3 Large" },
  { name: "Google: Gemini 2.5 Flash Lite Preview 06-17 (free)", provider: "Google", modelName: "Gemini 2.5 Flash Lite Preview 06-17" },
  { name: "Google: Gemini 2.5 Pro Preview 06-05 (free)", provider: "Google", modelName: "Gemini 2.5 Pro Preview 06-05" },
  ...
  // Скриншот 13: Perplexity, Mistral, OpenAI, ReMM, Noromaid
  ...
  { name: "Mistral Large 2411 (free)", provider: "Mistral", modelName: "Large 2411" },
  { name: "Mistral Large 2407 (free)", provider: "Mistral", modelName: "Large 2407" },
  { name: "Mistral: Pixtral Large 2411 (free)", provider: "Mistral", modelName: "Pixtral Large 2411" },
  { name: "Mistral Large (free)", provider: "Mistral", modelName: "Large" },
  ...
  // Дополнительные модели из скриншотов (без "(free)" в названии, но все бесплатные)
  { name: "Google: Gemini 2.5 Flash Image Preview (Nano Banana)", provider: "Google", modelName: "Gemini 2.5 Flash Image Preview (Nano Banana)" },
  { name: "Qwen2.5 Coder 32B Instruct (free)", provider: "Qwen", modelName: "2.5 Coder 32B Instruct" },
  { name: "Qwen2.5 72B Instruct (free)", provider: "Qwen", modelName: "2.5 72B Instruct" },
  { name: "Qwen2.5 VL 32B Instruct (free)", provider: "Qwen", modelName: "2.5 VL 32B Instruct" },
  { name: "Dolphin3.0 Mistral 24B (free)", provider: "Dolphin", modelName: "3.0 Mistral 24B" },
  { name: "SorcererLM 8x22B (free)", provider: "SorcererLM", modelName: "8x22B" },
  { name: "ReMM SLERP 13B (free)", provider: "ReMM", modelName: "SLERP 13B" },
  { name: "Noromaid 20B (free)", provider: "Noromaid", modelName: "20B" },
...
<head>
    ...
    <title>OpenRouter ALL Models - Complete List from Screenshots</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>OpenRouter ALL Models</h1>
            <p>Complete list of ALL models from screenshots (all are FREE)</p>
        </div>
        ...
        <div class="footer">
            ...
            <p>Data source: OpenRouter.ai Screenshots | Generated by BOSS-AI Complete Parser</p>
        </div>
    </div>
    ...
    <script>
        ...
        function exportToJSON() {
            ...
            a.href = url;
            a.download = 'openrouter-all-models.json';
...
function main() {
  ...
  console.log(`📊 Найдено ${allModelsFromScreenshots.length} моделей из ВСЕХ скриншотов`);

  // Удаляем дубликаты
  const uniqueModels = removeDuplicates(allModelsFromScreenshots);
  console.log(`✅ После удаления дубликатов: ${uniqueModels.length} уникальных моделей`);

  // Создаем детальную информацию для каждой модели
  const detailedModels = uniqueModels.map(model => createModelDetails(model));

  console.log('✅ Создана детальная информация для всех моделей');

  // Создаем HTML файл
  const htmlContent = createHTMLFile(detailedModels);
  fs.writeFileSync('openrouter-all-models.html', htmlContent);
  console.log('✅ Создан HTML файл: openrouter-all-models.html');

  // Создаем JSON файл
  const jsonContent = createJSONFile(detailedModels);
  fs.writeFileSync('openrouter-all-models.json', jsonContent);
  console.log('✅ Создан JSON файл: openrouter-all-models.json');
  ...
  console.log('   - openrouter-all-models.json (данные для API)');
}
