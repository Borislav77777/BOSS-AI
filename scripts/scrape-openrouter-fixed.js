...
/**
 * ИСПРАВЛЕННЫЙ парсер OpenRouter с правильным поиском результатов
 * - Ищет каждую модель через поисковую строку
 * - Находит результаты в правильном контейнере
 * - Получает РЕАЛЬНЫЕ ссылки из результатов поиска
 */

// Список моделей для поиска (первые 5 для тестирования)
...
async function parseModel(modelName) {
  try {
    ...
    await page.goto(realLink, { waitUntil: 'domcontentloaded', timeout: 30000 });
 ...
 */
async function main() {
  log('🚀 Запуск ИСПРАВЛЕННОГО парсера OpenRouter');
  ...
  fs.writeFileSync('openrouter-fixed-models.json', JSON.stringify(jsonData, null, 2));
  log(`✅ Сохранено ${results.length} моделей в openrouter-fixed-models.json`);
